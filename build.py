#!/usr/bin/env python3
"""Build the site from JSON + folders.

  links.json       site title/tagline/footer, home-page carousel, external links
  characters.json  one entry per character
  materials/photos/people/<slug>/   photos for that character (auto-discovered)
  materials/...                     anything else, listed under Files
...
PEOPLE = MATERIALS / "photos" / "people"
SITE = ROOT / "site"
...
CSS = """
  ...
  * { box-sizing: border-box; }
  ...
  .back {
    display: inline-block; margin-bottom: 1.5rem; color: var(--muted);
  ...
  .crole { display: block; color: var(--muted); font-size: .92rem; margin-top: .3rem; }
  ...
  .gallery figcaption { color: var(--muted); font-size: .8rem; margin-top: .35rem; }
...
def photo_index():
    """{slug: [photo]} joining photos.json (many-to-many) with per-person folders."""
    idx = {}
    seen = {}

    for ph in load(PHOTOS, {}).get("photos", []):
        src = "/" + ph["src"].lstrip("/")
        for slug in ph.get("people", []):
            entry = {"src": src, "caption": ph.get("caption", ""), "id": ph.get("id", "")}
            idx.setdefault(slug, []).append(entry)
            seen.setdefault(slug, set()).add(src)

    # Folder shortcut: materials/photos/people/<slug>/ implies that one person.
    if PEOPLE.is_dir():
        for folder in sorted(PEOPLE.iterdir()):
            if not folder.is_dir():
                continue
            slug = folder.name
            for p in sorted(folder.iterdir()):
                if not p.is_file() or p.suffix.lower() not in IMAGE_EXT or p.name in SKIP:
                    continue
                src = f"/photos/people/{slug}/{p.name}"
                if src in seen.get(slug, set()):
                    continue
                idx.setdefault(slug, []).append(
                    {"src": src, "caption": "", "id": "", "stem": p.stem}
                )
    return idx


def render_gallery(photos, captions):
    if not photos:
        return ""
    items = []
    for ph in photos:
        cap = ph.get("caption") or captions.get(Path(ph["src"]).name) or ""
        items.append(
            f'    <figure>\n      <img src="{html.escape(ph["src"])}" alt="{html.escape(cap)}" loading="lazy">\n'
...
def build_character_page(c, site_title, photos):
    slug = c["slug"]
    ...
    rel = f'<p class="tagline">{html.escape(c["relation"])}</p>' if c.get("relation") else ""
    role = f'<p class="tagline">{html.escape(c["role"])}</p>' if c.get("role") else ""

    paras = "\n".join(f"  <p>{html.escape(t)}</p>" for t in summary)
    if not paras:
        paras = '  <p class="note">No description yet.</p>'

    body = (
        f'  <a class="back" href="/index.html">&larr; {html.escape(site_title)}</a>\n'
        f"  <header>\n    <h1>{name}{aka}</h1>\n    {rel}{role}\n  </header>\n"
        f"{paras}\n"
        + render_gallery(photos, c.get("captions", {}))
    )
    out = SITE / "characters" / f"{slug}.html"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(page(f"{c['name']} — {site_title}", body))
    return len(photos)


def render_character_row(c, n_photos):
    name = html.escape(c["name"])
    aka = f' <span class="aka">&ldquo;{html.escape(c["aka"])}&rdquo;</span>' if c.get("aka") else ""
    rel = f'<span class="relation">{html.escape(c["relation"])}</span>' if c.get("relation") else ""
    role = f'<span class="crole">{html.escape(c["role"])}</span>' if c.get("role") else ""
    has_content = bool(c.get("summary")) or n_photos
    if has_content:
        count = f'<span class="count">{n_photos} photo{"s" if n_photos != 1 else ""}</span>' if n_photos else ""
        return (f'      <li><a href="/characters/{html.escape(c["slug"])}.html">\n'
                f'        <span class="cname">{name}{aka}</span>{rel}{count}{role}\n      </a></li>')
    return (f'      <li class="inert">\n        <span class="cname">{name}{aka}</span>{rel}'
            f'<span class="soon">pending</span>{role}\n      </li>')
...
def render_link(title, href, description, badge=None, meta=None):
    ...
    return (f'      <li>\n        <a href="{html.escape(href)}">\n'
            f'          <span class="row">{badge_html}<span class="title">{html.escape(title)}</span>{meta_html}</span>\n'
            f"          {desc_html}\n        </a>\n      </li>")
...
def main():
    ...
    blocks = [
        ...
        f'    <p class="tagline">{html.escape(site.get("tagline", ""))}</p>\n'
        "  </header>"
    ]
    ...
    if slides:
        figures = "\n".join(
            f'      <figure>\n        <img src="{html.escape(s["src"])}" '
            f'alt="{html.escape(s.get("caption") or title)}"{"" if i == 0 else " loading=lazy"}>\n'
        ...
        if len(slides) > 1:
            dots = "\n".join(
                ...
                f'aria-label="Photo {i + 1} of {len(slides)}"></button>' for i in range(len(slides))
            )
            nav = ('    <button class="arrow prev" aria-label="Previous photo">&lsaquo;</button>\n'
                   '    <button class="arrow next" aria-label="Next photo">&rsaquo;</button>\n'
                   f'    <div class="dots">\n{dots}\n    </div>\n')
        blocks.append(f'  <section class="carousel" aria-roledescription="carousel">\n'
                      f'    <div class="track">\n{figures}\n    </div>\n{nav}  </section>')

    # Characters: build a page each, then the index list.
    ...
    if people:
        ...
        pidx = photo_index()
        for c in people:
            photos = pidx.get(c["slug"], [])
            n = build_character_page(c, title, photos)
            total_photos += n
            rows.append(render_character_row(c, n))
        note = chars.get("note")
        blocks.append(
    ...
    # Everything else in materials/.
    mat_cfg = cfg.get("materials", {})
    found = scan_materials(exclude_dirs=tuple(mat_cfg.get("exclude", [])) + ("people",))
    if found:
    ...
    blocks.append(f'  <footer>{html.escape(site.get("footer", ""))}</footer>')

    SITE.mkdir(exist_ok=True)
    (SITE / "index.html").write_text(page(title, "\n".join(blocks), CAROUSEL_JS))
    n_files = sum(len(v) for v in found.values())
    print(f"Wrote site/index.html + {len(people)} character page(s) — "
          f"{total_photos} character photo(s), {n_files} other file(s).")
